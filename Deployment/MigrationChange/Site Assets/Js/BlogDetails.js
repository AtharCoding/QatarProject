	...
	$.when(get_Current_Blog, get_Current_Blog_Attch, get_NextBlog, get_LatestBlog)
		.then(function (respCurrentBlog, respCurrentAttchments, respNextBlog, respLatestBlog) {
			try {
				...
				let blogTitle=isArabic ? _currentBlog.BlogArabicTitle : _currentBlog.Title;
				let blogDesc=isArabic ? _currentBlog.BlogContentArabic : _currentBlog.BlogContent;
				_currentAttCollection = respCurrentAttchments[0].d.results;
				$("#blogTitle,#contentBlogTitle,#carouselBlogTitle").text(blogTitle);
				$("#blogDate,#carouselBlogDate").text(getFormattedDate(_currentBlog.ContentDate));
				$("#currentBlogContent").html(blogDesc);
				...
				getImageUrl(_currentBlog, 0, function (resultImgUrl, eachBlog) {
					...
					//For sharing site
					let metaTitle=SlicingTitle(blogTitle);
					let metaDesc=SlicingDesc(blogDesc);
					let metaImageUrl=resultImgUrl;
