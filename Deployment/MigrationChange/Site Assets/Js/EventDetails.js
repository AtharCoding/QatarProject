		...
		.then(function (respCurrentEvent, respCurrentAttchments) {
			try {
				...
				getImageUrl(_currentEvent, 0, function (resultImgUrl, eachEvent) {
					...
					$("head").append("<meta property='og:image' content='"+resultImgUrl+"'>");
				}, failure);
				...
				//For sharing site
				let metaTitle=SlicingTitle(eventTitle);
				let metaDesc=SlicingDesc($("<div></div>").append(currentEventDesc).text());
